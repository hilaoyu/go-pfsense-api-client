...
func (s DHCPService) ListLeases() ([]*DHCPLease, error) {
	response, err := s.client.get(leasesEndpoint, nil)
	if err != nil {
		return nil, err
	}
...
func (s DHCPService) ListStaticMappings(netInterface string) ([]*DHCPStaticMapping, error) {
	queryMap := map[string]string{
		"interface": netInterface,
	}
	response, err := s.client.get(staticMappingEndpoint, queryMap)
	if err != nil {
		return nil, err
	}
...
func (s DHCPService) CreateStaticMapping(newStaticMapping DHCPStaticMappingRequest) error {
	jsonData, err := json.Marshal(newStaticMapping)
	if err != nil {
		return err
	}
	_, err = s.client.post(staticMappingEndpoint, nil, jsonData)
	if err != nil {
		return err
	}
	return nil
